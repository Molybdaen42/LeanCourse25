...
lemma orbitOf_eq_iff (x y : X) : orbitOf G x = orbitOf G y ↔ y ∈ orbitOf G x := by
  ...
  · intro h
    ...
    use 1
    simp
  · intro ⟨g,hg⟩ -- *TA Comment:* mind the spacing: `⟨g, hg⟩`
    ext z
    simp [orbitOf] at * -- *TA Comment:* you don't need the `at *`
    rw [← hg] -- *TA Comment:* combine this with the `simp` above
    constructor
    · intro ⟨g',hg'⟩
      use g' • g⁻¹ -- *TA Comment:* this is scalar multiplication in `G`, it's the same as `*` but
...
def stabilizerOf (x : X) : Subgroup G where
  carrier := {g : G | g • x = x}
  mul_mem' h1 h2 := by -- *TA Comment:* you can write `mul_mem' {a b} h1 h2` to give them a name
    simp at *
    nth_rw 2 [← h1, ← h2]
...
def orbit_stabilizer_theorem (x : X) : G ⧸ stabilizerOf G x ≃ orbitOf G x := by
  ...
  let f' : G ⧸ stabilizerOf G x → orbitOf G x := Quotient.lift f (by
    intro a b hab
    simp [stabilizerOf] at hab -- *TA Comment:* you can simply do `simp at hab`
    simp [f]
    nth_rw 1 [← hab, ← smul_eq_mul, smul_assoc, smul_inv_smul])
  apply Equiv.ofBijective f'
  constructor
  · -- injectivity
    -- *TA Comment:* use `Quotient.ind₂` here
    intro g₁ g₂ hfg₁_eq_fg₂
    simp [f', f] at hfg₁_eq_fg₂
    sorry
  · -- surjectivity
    intro ⟨y,⟨g,hgy⟩⟩ -- *TA Comment:* mind the spacing
    simp at hgy -- *TA Comment:* this is not needed
    use g
...
example (u : ℕ → ℝ) (x : ℝ) : MyTendsto u atTop (𝓝 x) ↔ ∀ ε > 0, ∃ N, ∀ n ≥ N, |u n - x| < ε := by
  ...
  · intro h ε hε
    have : ∃ N, ∀ n ≥ N, n ∈ u ⁻¹' (Ioo (x - ε) (x + ε)) := by
      ...
      -- *TA Comment:* you're repeatedly using rewrite lemmas (`eventually_atTop`, `mem_nhds_iff`)
      -- with `apply`. Why not just rewrite with them?
      use Ioo (x-ε) (x+ε)
      simp [hε, isOpen_Ioo]
      rfl
...
lemma technical_filter_exercise {ι α : Type*} {p : ι → Prop} {q : Prop} {a b : α}
  ...
  constructor
  · intro h1 r h2
    apply Filter.Eventually.mono h1 -- *TA Comment:* this can be written shorter as `apply h1.mono`
    intro x hx
    simp [hx] -- *TA Comment:* `rw` can do this
    by_cases hq : q
    · simp [hq] at *
      exact haF h2
    · simp [hq] at *
