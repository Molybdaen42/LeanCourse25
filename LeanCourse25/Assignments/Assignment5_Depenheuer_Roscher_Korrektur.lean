...
lemma finset_card_powerset (α : Type*) [DecidableEq α] (s : Finset α) :
  ...
  | insert x s hxs ih =>
      have :  Disjoint s.powerset (Finset.image (insert x) s.powerset) := by
        ...
        /- *TA Comment:* You don't really need this case distinction since you can prove `x ∈ u`
        easily with `simp [← hxvu]`. -/
        by_cases hxu : x ∈ u
        · exact hxs (hus hxu)
        · simp [← hxvu] at hxu
      ...
      replace ht₂ : x ∉ t₂ := by exact fun a ↦ hxs (ht₂ a)
      /- *TA Comment:* You can finish here with
      `rw [← Finset.erase_insert ht₁, h, Finset.erase_insert ht₂]`. -/
      ext y
      constructor
      · intro hyt₁
