...
lemma not_op_comm : ¬(∀ a b : Point4, op a b = op b a) := by
  push_neg
  use ⟨0, 0, 1, 0⟩
  use ⟨0, 1, 0, 0⟩ -- *TA Comment:* You can combine the two `use`s: `use ⟨0, 0, 1, 0⟩, ⟨0, 1, 0, 0⟩`
  simp [op]
  norm_num
...
noncomputable example : Group' SpecialPoint where
  ...
  /- *TA Comment:* You don't actually need to specify the type above, i.e. you can do
  `neutral := ⟨⟨1,0,0,0⟩, by norm_num⟩`. -/
  gop_neutral := by
    intro a
    simp [op', op]
    rfl
  inv := fun a ↦ ⟨⟨a.1.x,-a.1.y,-a.1.z,-a.1.w⟩, by simp [a.prop]⟩
  -- *TA Comment:* Instead of using `fun a ↦` put the `a` to the left of the `:=`
  gop_inv := by
    intro a
    simp [op', op]
    ring_nf
...
def groupPosRat : Group PosRat where
  ...
  /- *TA Comment:* above you can do `fun a b ↦ ⟨a*b, mul_pos a.prop b.prop⟩` but best would be
  `mul a b := ⟨a * b, mul_pos a.prop b.prop⟩`. -/
  mul_assoc := by
    intro a b c -- *TA Comment:* These too can go to the left of the `:=`
    ext
    simp -- *TA Comment:* you can do `simp [mul_assoc]` here to finish
    apply mul_assoc
  one := 1
  one_mul := by
    intro a
    ext -- *TA Comment:* this is not needed
    simp
  mul_one := by
    intro a
    ext -- *TA Comment:* this is not needed
  ...
  div_eq_mul_inv := by
    intro a b
    ext -- *TA Comment:* not needed
    rfl
  zpow_zero' := by
    intro a
    unfold zpowRec
    simp -- *TA Comment:* not needed
    rfl
  zpow_succ' := by
    intro n a
    rfl
...
/- *TA Comment:* I would write your lemma like this and then replace the `h` below with
`(fun a b ↦ (equiv_iff_sub_eq a b).1)` which arguably is a lot more complicated but
I think the lemma is a lot nicer like this. -/
lemma equiv_iff_sub_eq (a b : ℤ × ℤ):
    letI := integerEquivalenceRelation
    a ≈ b ↔ a.1 - a.2 = b.1 - b.2 := by
  simp [sub_eq_sub_iff_add_eq_add, add_comm]

-- **TA Points: 6/6**
example : Quotient integerEquivalenceRelation ≃ ℤ where
  toFun := Quotient.lift (fun ⟨k,l⟩ ↦ k-l) (fun a b ↦ (equiv_iff_sub_eq a b).1)
  invFun := fun k ↦ Quotient.mk integerEquivalenceRelation ⟨k,0⟩
  left_inv := by
    unfold LeftInverse -- *TA Comment:* not needed
    intro x
    simp -- *TA Comment:* not needed
    have h := Quotient.exists_rep x
    obtain ⟨ a, ha⟩  := h
    /- *TA Comment:* You can directly do `obtain ⟨a, ha⟩ := Quotient.exists_rep x` above. -/
    rw [← ha]
